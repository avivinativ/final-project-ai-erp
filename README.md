...
- ~~קרדנציאל Google Drive פג תוקף~~ - תוקן: המשתמש חיבר מחדש, ותוך כדי כך התגלה עוד באג
  אמיתי (ראו `app/README.md`, סעיף Status): `createFromText` תמיד מעלה כ-`text/plain` בלי קשר
  לסיומת הקובץ, כך שדרייב הציג את קובץ ה-HTML כטקסט גולמי. תוקן ע"י מעבר ל-node "Convert to
  File" עם `mimeType: text/html` מפורש ולאחריו `operation: upload` בינארי במקום `createFromText`.
  WF8 פעיל שוב ומאומת חי.
